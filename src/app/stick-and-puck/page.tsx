...
export default function StickAndPuckPage() {
  ...
  const dateRange = useMemo(
    () => Array.from({ length: 7 }, (_, i) => addDays(dateRangeStart, i)),
    [dateRangeStart],
  );
  ...
  const sessionsByRink = useMemo(() => {
    const map = new Map<
      ...
      {
        rinkName: string;
        location: SessionWithDistance['location'];
  ...
  // All session types present in the full data set
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={cn(
                          'text-sm font-bold',
                          !isSelected && !hasSessions && 'opacity-50',
                        )}
                      >
                        {format(date, 'd')}
                      </span>
                      {hasSessions && !isSelected && (
                        <div className="w-1 h-1 rounded-full mt-0.5 theme-primary-bg" />
                      )}
                    </button>
                  );
                })}
            ...
            {showFilters && (
              ...
              >
                <div className="px-4 py-3">
                  <p className="text-[10px] text-slate-500 font-medium uppercase mb-2">
                    Session Type
                  </p>
            ...
            <div className="mb-4 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-amber-700 mt-0.5">
                  ...
                  to set your home location for distance info.
                </p>
              </div>
            </div>
          )}
          ...
          {!loading && !error && sessionsForDate.length === 0 && (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm font-semibold text-slate-700">
                No sessions on {format(selectedDate, 'EEEE, MMM d')}
              </p>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <p className="text-xs text-slate-500">
                  ...
                  {sessionsForDate.length !== 1 ? 's' : ''} at{' '}
                  {sessionsByRink.length} rink
                  {sessionsByRink.length !== 1 ? 's' : ''}
                </p>
              </div>
                  ...
                  <div className="px-4 pt-3 pb-2 border-b border-slate-100">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 theme-primary-bg-faint">
                          <Snowflake size={16} className="theme-primary-text" />
                        </div>
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-2 text-[11px] text-slate-500">
                            ...
                            {rink.distance != null && (
                              <span className="flex items-center gap-0.5 font-medium theme-primary-text">
                                <Navigation size={10} />
                      ...
                      <span className="px-2 py-0.5 text-[10px] font-bold rounded-full bg-slate-100 text-slate-600">
                        {rink.sessions.length} session
                        {rink.sessions.length !== 1 ? 's' : ''}
                      </span>
                    </div>
                  </div>
            ...
            <div className="pt-3 mt-2 border-t border-slate-100 space-y-2">
              ...
              {session.recurring && (
                <div className="flex items-center gap-2 text-xs text-slate-500">
                  ...
                  <span>
                    Recurring every{' '}
                    {session.recurring.days.map((d) => DAY_NAMES[d]).join(', ')}
                  </span>
                </div>
              )}
              ...
              {session.registrationUrl && (
                <a
                  ...
                  className="inline-flex items-center gap-1.5 text-xs font-medium mt-1 theme-primary-text"
                >
