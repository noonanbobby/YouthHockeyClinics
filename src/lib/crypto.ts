...
async function importKey(): Promise<CryptoKey> {
  if (!isEncryptionConfigured()) throw new Error('CREDENTIAL_ENCRYPTION_KEY is not configured');
  const envKey = getEnvKey();
  const raw = new Uint8Array(32);
  for (let i = 0; i < 32; i++) raw[i] = parseInt(envKey.slice(i * 2, i * 2 + 2), 16);
...
export async function encryptSettingsCredentials(settings: Record<string, unknown>): Promise<Record<string, unknown>> {
  const out = { ...settings };

  if (out.daySmartConfig && typeof out.daySmartConfig === 'object') {
    const dsc = { ...(out.daySmartConfig as Record<string, unknown>) };
    if (typeof dsc.password === 'string' && dsc.password) dsc.password = await encryptCredential(dsc.password);
  ...
  if (out.iceHockeyProConfig && typeof out.iceHockeyProConfig === 'object') {
    ...
    if (typeof ihp.password === 'string' && ihp.password) ihp.password = await encryptCredential(ihp.password);
  ...
  if (out.emailScanConfig && typeof out.emailScanConfig === 'object') {
    ...
    if (typeof esc.password === 'string' && esc.password) esc.password = await encryptCredential(esc.password);
    if (typeof esc.accessToken === 'string' && esc.accessToken) esc.accessToken = await encryptCredential(esc.accessToken);
    if (typeof esc.refreshToken === 'string' && esc.refreshToken) esc.refreshToken = await encryptCredential(esc.refreshToken);
...
export async function decryptSettingsCredentials(settings: Record<string, unknown>): Promise<Record<string, unknown>> {
  const out = { ...settings };
  ...
  if (out.emailScanConfig && typeof out.emailScanConfig === 'object') {
    ...
    if (typeof esc.accessToken === 'string') esc.accessToken = await decryptCredential(esc.accessToken);
    if (typeof esc.refreshToken === 'string') esc.refreshToken = await decryptCredential(esc.refreshToken);
    out.emailScanConfig = esc;
  }
